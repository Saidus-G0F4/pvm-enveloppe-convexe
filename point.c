...
point *point_random(int nbPts)/* nombre de points a` tirer */
{
	...
	for(i=0; i < nbPts; i++) {
		...
		do {
			pts[i]->x = rand() % XMAX;
			for(j=0; j<i && pts[i]->x!=pts[j]->x; j++); // fait en sorte que les abscisses soient differentes
		} while( j != i );
		do {
			pts[i]->y = rand() % YMAX;
			for(j=0; j<i && pts[i]->y!=pts[j]->y; j++); // fait en sorte que les ordonnees soient differentes
		} while( j != i );
	}
